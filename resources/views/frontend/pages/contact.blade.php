...
<section class="pt-24 pb-44 bg-white">
    <div class="max-w-7xl mx-auto px-4">
        <div class="text-center mb-16 space-y-4">
            <span class="text-primary font-black uppercase tracking-[0.2em] text-xs">{{ __('Get In Touch') }}</span>
            <h2 class="text-4xl lg:text-5xl font-black text-slate-900 tracking-tighter uppercase italic">{{ __('Contact Us') }}</h2>
            <div class="w-20 h-1.5 bg-primary rounded-full mx-auto"></div>
        </div>

        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 max-w-5xl mx-auto">
            <div class="bg-slate-50 p-12 rounded-[40px] space-y-8">
                <h3 class="text-2xl font-black tracking-tighter">{{ __('Reach Out to Us') }}</h3>
                <p class="text-slate-500 font-medium">{{ __('Have a question about our products, an order, or something else? We\'d love to hear from you.') }}</p>
            ...
            <form class="space-y-6 bg-white p-12 rounded-[40px] shadow-2xl border border-slate-100">
                <div class="space-y-2">
                    <label class="text-xs font-black uppercase tracking-widest text-slate-400">{{ __('Full Name') }}</label>
                ...
                <div class="space-y-2">
                    <label class="text-xs font-black uppercase tracking-widest text-slate-400">{{ __('Email Address') }}</label>
                ...
                <div class="space-y-2">
                    <label class="text-xs font-black uppercase tracking-widest text-slate-400">{{ __('Message') }}</label>
                ...
                <button type="button" class="w-full py-5 bg-primary hover:bg-primary-dark text-white font-black uppercase tracking-widest text-xs rounded-2xl transition-all shadow-xl shadow-primary/30 active:scale-95">{{ __('Send Message') }}</button>
            </form>
        </div>
    </div>
</section>
